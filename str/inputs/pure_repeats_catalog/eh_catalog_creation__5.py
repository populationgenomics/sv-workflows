"""
This script creates a .JSON catalog compatible with ExpansionHunter.
The input is a BED file containing loci of pure_repeats, which was
...
# manually add off target regions based on original Illumina catalog
...
with open(PURE_BED, encoding='utf-8') as handle:
    for line in handle:
        chr, start, end, motif_length, motif, num_repeats = line.split('\t')
        variant_id = f'{chr}_{start}_{end}'
        locus_structure = f'({motif})*'
        reference_region = f'{chr}:{start}-{end}'

        # chr9:27573528-27573546 is a special locus because Illumina catalog
        # specifies Off-Target-Regions; changes classification to RareRepeat
        entry: dict = {
            'LocusId': variant_id,
        ...
        # which also changes classification to RareRepeat
        if reference_region == 'chr9:27573528-27573546':
            entry['OfftargetRegions'] = chr9_27573528_27573546_off_target_regions

        eh_catalog.append(entry)

print(f'eh_catalog (164847): {len(eh_catalog)}')
with open(PURE_JSON_OUTPUT, 'w', encoding='utf-8') as eh:
