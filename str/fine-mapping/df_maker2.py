...
def run_concatenator(cell):
    ...
    for chrom in range(1,23):
        try:
            files_list = list(to_path(f'gs://cpg-bioheart-test-analysis/str/associatr/fine_mapping/v3-ocv/susie/{cell}/chr{chrom}').glob('*.tsv'))
        except:
            continue
        for file in files_list:
            ...
            data['celltype'] = cell
            data['gene'] = gene_name
            data = data[~data['motif'].str.contains('-')] # remove SNP rows
            dfs.append(data)
        print (f'Processed {cell} and chr{chrom}')
    result_df = pd.concat(dfs, ignore_index=True)
    result_df.to_csv(f'gs://cpg-bioheart-test-analysis/str/associatr/fine_mapping/v3-ocv/susie/{cell}/all_genes_strs.tsv', sep = '\t', index = False)
