...
def dosages(df, snp_input, tr_input):
    import pandas as pd
    import numpy as np
    ...
    for row in df.itertuples(): #for every row ie for every gene - pull out the variants GT in the cis window.
...
def main(estrs_path):
    ...
    for chrom in df['chr'].unique():
        ...
        j = b.new_python_job(f'Prepare for {chr}')
        j.cpu(0.25)
        j.storage('5G')
        j.call(dosages, df_chr, snp_input, tr_input)
