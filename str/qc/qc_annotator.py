...
analysis-runner --access-level "test" --dataset "bioheart" --description "QC annotator" --output-dir "str/polymorphic_run/mt/bioheart_tob/v1_bioheart_n975" qc_annotator.py \
...
def main(mt_path):
    ...
    ht = mt.select_rows(
        alleles_rep_lengths=hl.agg.collect(mt.allele_1_rep_length).extend(hl.agg.collect(mt.allele_2_rep_length)),
    ).rows()
    ...
    max_key_expr = hl.bind(
        lambda d: hl.bind(lambda kv: kv[0], hl.sorted(d.items(), key=lambda kv: -kv[1])[0]),
    ...
    mt = mt.annotate_rows(
        prop_alleles_is_not_mode=mt.sum_alleles_is_not_mode / hl.sum(mt.aggregated_info.allele_array_counts.values()),
    )
    ...
    mt = mt.annotate_entries(
        allele_1_REPCI_over_CN=hl.float64(mt.allele_1_REPCI_size / mt.allele_1_rep_length),
