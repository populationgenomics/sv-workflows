...
def job_batcher(egenes_cell_type, gwas_file, pheno_output_name, snp_cis_dir, celltype):
    ...
    for gene in egenes_cell_type['gene_name']:
        #get the chromosome for the gene
        chrom = egenes_cell_type[egenes_cell_type['gene_name'] == gene]['chr'].iloc[0]
        if to_path(
            output_path(
        ...
        if to_path(f'{snp_cis_dir}/{celltype}/{chrom}/{gene}_100000bp_meta_results.tsv').exists():
            ...
            # run coloc
            coloc_runner(hg38_map_chr_start_end, f'{snp_cis_dir}/{celltype}/{chrom}/{gene}_100000bp_meta_results.tsv', celltype, pheno_output_name)
        else:
...
def main(snp_cis_dir, egenes_file, celltypes, pheno_output_name, max_parallel_jobs, job_cpu):
    ...
    for celltype in celltypes.split(','):
        ...
        gwas_file = f'gs://cpg-bioheart-test/str/gymrek-ukbb-snp-str-gwas-catalogs_v6/white_british_{phenotype}_snp_str_gwas_results_hg38.tab.gz'


        # run coloc
        coloc_job = b.new_python_job(
            f'Coloc for {celltype}: {phenotype}',
        )
