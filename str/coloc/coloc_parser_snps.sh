...
for pheno in "${pheno_names[@]}"; do
    ...
    --access-level "test" \
    --output-dir "str/associatr/final_freeze/meta_fixed/v6/coloc-snp-only/sig_str_and_gwas_hit" \
    coloc_results_parser.py \
    --coloc-dir=gs://cpg-tenk10k-test-analysis/str/associatr/final_freeze/meta_fixed/v6/coloc-snp-only/sig_str_and_gwas_hit \
    --celltypes "$celltypes" \
    --phenos="$pheno"
done
