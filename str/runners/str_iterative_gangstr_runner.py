...
from cpg_utils.hail_batch import output_path, reference_path, remote_tmpdir
...
@click.command()
def main(variant_catalog, dataset, external_wgs_ids: list[str]):  # pylint: disable=missing-function-docstring
    # Initializing Batch
    ...
    if cpg_sids_without_crams:
        external_wgs_sids_without_crams = ', '.join(cpg_id_to_external_id[sid] for sid in cpg_sids_without_crams)
        logging.warning(f'There were some samples without CRAMs: {external_wgs_sids_without_crams}')
