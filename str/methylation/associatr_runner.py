...
def main():
    ...
    for chromosome in get_config()['associatr']['chromosomes'].split(','):
        ...
        for i in range(0, len(site_numpy_list), 5000):
            _dependent_jobs = []
            reset_batch()
            batch_gene_files = site_numpy_list[i : i + 5000]
            b = get_batch(name='Run associatr-methylation chr' + chromosome + ' sites ' + str(i) + '-' + str(i + 5000))
