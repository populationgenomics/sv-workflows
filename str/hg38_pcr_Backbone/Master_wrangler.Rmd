...
unlist(split_keys[1]) 
fasta_with_capillary_eh_gangstr %>% select(key, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",5:6)  %>%filter(key %in%unlist(split_keys[6])  ) %>%ggplot(aes(x = key, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
