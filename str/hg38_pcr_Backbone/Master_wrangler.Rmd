...
fasta_with_capillary_eh_gangstr %>% filter(key.x == "GAT004" & cpg_id == "CPG154138")
...
fasta_with_capillary_eh_gangstr_al$caller_minus_capillary = factor(fasta_with_capillary_eh_gangstr_al$caller_minus_capillary, levels = c("max_e_allele_diff", "min_e_allele_diff", "max_g_allele_diff", "min_g_allele_diff", "max_h_allele_diff", "min_h_allele_diff"))


fasta_with_capillary_eh_gangstr_al%>%filter(key.x %in% unlist(split_keys[12])) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
geom_boxplot()+ theme_minimal()+ theme(panel.grid.minor = element_blank())+ xlab("locus")  +labs(color='Discrepancy') +ylab("Difference (num.repeats) from capillary")+ scale_y_continuous(breaks=seq(-15,0,1))


reference_graph_al = fasta_with_capillary_eh_gangstr_al  %>% select(key.x, max_e_allele,min_e_allele,max_g_allele,min_g_allele,max_h_allele,min_h_allele, max_cap_allele, min_cap_allele,trf_num_repeats) %>% mutate(`EH-ref_1` = max_e_allele-trf_num_repeats,`EH-ref_2` = min_e_allele-trf_num_repeats,`GangSTR-ref_1` = max_g_allele-trf_num_repeats,`GangSTR-ref_2` = min_g_allele-trf_num_repeats, `HipSTR-ref_1` = max_h_allele-trf_num_repeats,`HipSTR-ref_2` = min_h_allele-trf_num_repeats, `capillary-ref_1` = max_cap_allele-trf_num_repeats, `capillary-ref_2` = min_cap_allele-trf_num_repeats) %>% gather(key = "caller_minus_capillary", value = "difference",11:18) 
...
```{r}
pearson_experiment = fasta_with_capillary_eh_gangstr %>% filter(!is.na(capillary_alleles))

fasta_with_capillary_eh_gangstr %>% distinct(key.x)

pearson_experiment= pearson_experiment %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles,`HipSTR-capillary` = h_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(mode_EH_capillary = getmode(`EH-capillary`),mode_GangSTR_capillary = getmode(`GangSTR-capillary`), mode_HipSTR_capillary = getmode(`HipSTR-capillary`), median_EH_capillary = median(`EH-capillary`), median_GangSTR_capillary = median(`GangSTR-capillary`), median_HipSTR_capillary = median(`HipSTR-capillary`)) %>% ungroup() %>% mutate(is_mode_EH_capillary = case_when(mode_EH_capillary == `EH-capillary` ~ 1, TRUE ~0), is_mode_GangSTR_capillary = case_when(mode_GangSTR_capillary == `GangSTR-capillary` ~ 1, TRUE ~0), is_mode_HipSTR_capillary = case_when(mode_HipSTR_capillary == `HipSTR-capillary` ~ 1, TRUE ~0)) %>% group_by(key.x) %>% mutate(mode_EH_capillary_proportion = sum(is_mode_EH_capillary)/n(), mode_GangSTR_capillary_proportion = sum(is_mode_GangSTR_capillary)/n(),mode_HipSTR_capillary_proportion = sum(is_mode_HipSTR_capillary)/n())  %>%  mutate(capillary_alleles = case_when(mode_GangSTR_capillary==mode_EH_capillary & mode_EH_capillary == mode_HipSTR_capillary  & mode_EH_capillary_proportion >=0.5 &mode_HipSTR_capillary_proportion >=0.5 &mode_GangSTR_capillary_proportion >=0.5 &mode_GangSTR_capillary!=0 &mode_HipSTR_capillary == median_HipSTR_capillary & mode_EH_capillary == median_EH_capillary  ~ NA, TRUE ~ capillary_alleles))

pearson_experiment = pearson_experiment %>% filter(is.na(capillary_alleles))

pearson_experiment %>% distinct(key.x, trf_num_repeats, mode_EH_capillary) %>% ggplot(aes(x=trf_num_repeats, mode_EH_capillary)) + geom_point() + xlab("Num. repeats on hg38 backbone") + ylab("Offset (num.repeats)") + theme_minimal()

cor_data = pearson_experiment %>% distinct(key.x, trf_num_repeats, mode_EH_capillary)

cor(cor_data$trf_num_repeats, cor_data$mode_EH_capillary)
