...
gangstr %>% filter(catalog_start_coordinate_0_based ==98177578) %>% filter(sample_id=="CPG148163")
...
eh_gangstr %>% filter(catalog_start_coordinate_0_based ==98177578) %>% filter(cpg_id=="CPG148163")
...
```{r}
hipstr = read.csv("~/Downloads/str_410_sgdp_loci_data-frames_hipstr (1).csv")%>% separate(h_gb, c("h_gb_1", "h_gb_2"), sep = "\\|") %>% mutate(h_gb_1 = as.numeric(h_gb_1), h_gb_2 = as.numeric(h_gb_2)) %>% mutate(h_allele_1 = (nchar(h_ref)+h_gb_1)/h_period, h_allele_2 = (nchar(h_ref)+h_gb_2)/h_period, h_alleles = (h_allele_1) + (h_allele_2), h_gbs = h_gb_1 + h_gb_2) %>% rename("key"= h_id,"hgdp_sample_id"= sample_id) %>% rename("end" = h_end, "chr" = h_chr)

hipstr %>% filter(key =="GAAT1A4") %>% filter(hgdp_sample_id=="HGDP00540")
...
fasta_with_capillary_eh_gangstr = fasta_with_capillary_eh_gangstr %>% mutate(e_alleles = as.numeric(e_allele_1)+as.numeric(e_allele_2), 
                 ...
                 capillary_alleles = (capillary_allele_1) + (capillary_allele_2)) #summed diploid genotypes 
```
...
```
```{r}
fasta_with_capillary_eh_gangstr%>% group_by(key.x)%>% filter(e_alleles==capillary_alleles)%>% summarise(n=n()) %>% arrange(desc(n))


fasta_with_capillary_eh_gangstr%>% filter(key.x == "GAAT1A4") %>% filter(cpg_id=="CPG148163")
```
...
```{r}
fasta_with_capillary_eh_gangstr%>% filter(key.x %in% c("GGAA20F08")) %>% select(e_alleles, h_alleles, g_alleles, capillary_alleles, h_gbs, PCR_summed_diffs) %>% mutate(tester =h_gbs-PCR_summed_diffs) %>% select(tester)


keys_of_minus_2 = fasta_with_capillary_eh_gangstr %>% select(key, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-2) %>% distinct(key)

keys_100_concord = fasta_with_capillary_eh_gangstr %>% select(key, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-0) %>% distinct(key)


fasta %>% filter(key %in% c("GAT004") )
```
