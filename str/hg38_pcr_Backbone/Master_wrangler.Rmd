...
```
```{r}
hipstr = read.csv("~/Downloads/str_410_sgdp_loci_data-frames_410_sgdp_loci_hipstr.csv")%>% separate(h_gb, c("h_gb_1", "h_gb_2"), sep = "\\|") %>% mutate(h_gb_1 = as.numeric(h_gb_1), h_gb_2 = as.numeric(h_gb_2)) %>% mutate(h_allele_1 = (nchar(h_ref)+h_gb_1)/h_period, h_allele_2 = (nchar(h_ref)+h_gb_2)/h_period, h_alleles = floor(h_allele_1) + floor(h_allele_2)) %>% rename("key"= h_id,"hgdp_sample_id"= sample_id) %>% select(hgdp_sample_id, key, h_allele_1, h_allele_2, h_alleles)

hipstr %>% select(hgdp_sample_id) %>%as.vector()%>% write.table("hgdp_sample_ids.txt", sep = ",", row.names = FALSE)

merge(fasta_with_capillary_eh_gangstr, hipstr, by =c("hgdp_sample_id", "key")) %>% distinct(hgdp_sample_id)
```
...
(fasta_with_capillary_eh_gangstr %>% filter(e_alleles== capillary_alleles|e_alleles+2 == capillary_alleles|e_alleles+4 == capillary_alleles|e_alleles+6 == capillary_alleles) %>%count())/ (fasta_with_capillary_eh_gangstr %>% count())
...
library(ggbeeswarm)
library(ggforce)
my_vec = fasta_with_capillary_eh_gangstr %>% distinct(key)
my_vec = my_vec$key
split_keys = split(my_vec,             # Applying split() function
      ceiling(seq_along(my_vec) / 50))
unlist(split_keys[1]) 
fasta_with_capillary_eh_gangstr %>% select(key, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",5:6)  %>%filter(key %in%unlist(split_keys[1])  ) %>%ggplot(aes(x = key, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
geom_boxplot()+ theme_minimal()+ theme(panel.grid.minor = element_blank())+ xlab("locus")  +labs(color='Discrepancy') +ylab("Difference (num.repeats) from capillary")+ scale_y_continuous(breaks=seq(-30,10,1))
```
```{r}
# Create the function.
getmode <- function(v) {
...
fasta %>% filter(key %in% keys_of_minus_2$key) 
```


```{r}
fasta%>% filter(key %in% keys_100_concord$key)
```
