---
title: "PCR validation work for HGDP dataset"
author: "Hope"
date: "15/02/2023"
output: html_document
---

This R Markdown file consists of data wrangling and analysis work to validate HipSTR, GangSTR, and EH calls against PCR dataset from Rosenberg et al. (2005).
...
## TRF output (metrics) for each target PCR amplicon 
```{r}
...
```
`
## PCR sequence of each target PCR amplicon 
```{r}
...
We now wrangle the data to derive new variables 
```{r}
...
## Redefine STR motifs on the antisense strand 
```{r}
...
```
# Catalog creation for GangSTR and HipSTR based on hg38 
```{r}
...
# Ingestion of Capillary/PCR genotypes 
```{r}
...
#Exclude PCR calls identified to have indels in the amplicon region as per FermiKit
```{r}
...
mappings = read.table("~/Desktop/untitled folder/210_sample_mappings.txt", sep = ",") %>% rename("hgdp_sample_id" = V1, "cpg_id" = V2)

hipstr = merge(hipstr, mappings, by="hgdp_sample_id", all.x = TRUE)

fasta_with_capillary_trio = merge(fasta_with_capillary_eh_gangstr, hipstr, by =c("cpg_id", "chr", "end"), all.x = TRUE)

fasta_with_capillary_trio=fasta_with_capillary_trio %>% mutate(h_allele_1 = trf_num_repeats +h_gb_1/h_period, h_allele_2 = trf_num_repeats +h_gb_2/h_period, h_alleles = floor(h_allele_1) + floor(h_allele_2), h_gbs = h_gb_1 + h_gb_2)
```

#Calculate PCR allele lengths from bp result 
```{r}
fasta_with_capillary_trio = fasta_with_capillary_trio %>% mutate(capillary_allele_1 = (trf_num_repeats) + (PCR_1 -PCR_length)/nchar(trf_motif),
                               capillary_allele_2 = (trf_num_repeats) + (PCR_2 -PCR_length)/nchar(trf_motif) )

fasta_with_capillary_trio = fasta_with_capillary_trio %>% mutate(e_alleles = as.numeric(e_allele_1)+as.numeric(e_allele_2), 
...
#out of the box concordance rates 
```{r}
fasta_with_capillary_trio =fasta_with_capillary_trio %>% filter(!is.na(capillary_alleles))

#remove loci with fewer than 20 EH calls (EH chosen because EH calls every locus in every sample) 
loci_to_remove = fasta_with_capillary_trio %>% group_by(key.x)%>% summarise(n=n()) %>% filter(n<20)
fasta_with_capillary_trio = fasta_with_capillary_trio %>% filter(!(key.x %in% loci_to_remove$key.x))

# out of the box internal consistency between callers
fasta_with_capillary_trio %>% filter(e_alleles == g_alleles) %>% count()/ (fasta_with_capillary_trio %>% count())
fasta_with_capillary_trio %>% filter(e_alleles == h_alleles) %>% count()/ (fasta_with_capillary_trio %>% count())
fasta_with_capillary_trio %>% filter(g_alleles == h_alleles) %>% count()/ (fasta_with_capillary_trio%>% count())

#out of the box concordance with capillary calls 
(fasta_with_capillary_trio %>% filter(e_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio%>% count())
(fasta_with_capillary_trio %>% filter(h_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio  %>% count())
(fasta_with_capillary_trio %>% filter(g_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio%>% count())

```

#Summed diploid genotype Visualisation (boxplots showing systematic shifts)
```{r}
#boxplots showing systematic shifts relative to capillary 
library(ggbeeswarm)
library(ggforce)
my_vec = fasta_with_capillary_trio %>% distinct(key.x)
my_vec = my_vec$key.x
split_keys = split(my_vec,             # Applying split() function
      ceiling(seq_along(my_vec) / 10))

fasta_with_capillary_trio  %>% select(key.x,trf_period, e_alleles,g_alleles,h_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles, `HipSTR-capillary` = h_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",7:9)  %>%filter(key.x %in% unlist(split_keys[6])) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
...
fasta_with_capillary_trio %>% filter(key.x == "GAT004" & cpg_id == "CPG154138")

```

```{r}
#boxplots showing systematic shifts relative to ref num_repeats
library(ggbeeswarm)
library(ggforce)
reference_graph = fasta_with_capillary_trio  %>% select(key.x, e_alleles,g_alleles,h_alleles,trf_num_repeats, capillary_alleles) %>% mutate(`EH-ref` = e_alleles-trf_num_repeats*2, `GangSTR-ref` = g_alleles-trf_num_repeats*2, `HipSTR-ref` = h_alleles-trf_num_repeats*2, `capillary-ref` = capillary_alleles-trf_num_repeats*2) %>% gather(key = "caller_minus_capillary", value = "difference",7:10) 
...
# Allele specific visualisation of systematic shifts

```{r}
#the allele with the greater length will be max_allele, and the other allele will be min_allele
fasta_with_capillary_trio_al = fasta_with_capillary_trio
fasta_with_capillary_trio_al$e_allele_1 = as.numeric(fasta_with_capillary_trio_al$e_allele_1)
fasta_with_capillary_trio_al$e_allele_2 = as.numeric(fasta_with_capillary_trio_al$e_allele_2)
fasta_with_capillary_trio_al$g_allele_1 = as.numeric(fasta_with_capillary_trio_al$g_allele_1)
fasta_with_capillary_trio_al$g_allele_2 = as.numeric(fasta_with_capillary_trio_al$g_allele_2)

fasta_with_capillary_trio_al$max_e_allele = apply(fasta_with_capillary_trio_al[, 5:6 ], 1, max)
fasta_with_capillary_trio_al$min_e_allele = apply(fasta_with_capillary_trio_al[, 5:6 ], 1, min)

fasta_with_capillary_trio_al$max_g_allele = apply(fasta_with_capillary_trio_al[,8:9],1,max)
fasta_with_capillary_trio_al$min_g_allele = apply(fasta_with_capillary_trio_al[,8:9],1,min)

fasta_with_capillary_trio_al$max_h_allele = apply(fasta_with_capillary_trio_al[,51:52],1,max)
fasta_with_capillary_trio_al$min_h_allele = apply(fasta_with_capillary_trio_al[,51:52],1,min)

fasta_with_capillary_trio_al$max_cap_allele = floor(apply(fasta_with_capillary_trio_al[,55:56],1,max))
fasta_with_capillary_trio_al$min_cap_allele = floor(apply(fasta_with_capillary_trio_al[,55:56],1,min))

#create 2 variables storing the difference in alleles
fasta_with_capillary_trio_al = fasta_with_capillary_trio_al %>% mutate(
...
fasta_with_capillary_trio_al=fasta_with_capillary_trio_al %>% gather(key = "caller_minus_capillary", value = "difference",71:76)

fasta_with_capillary_trio_al$caller_minus_capillary = factor(fasta_with_capillary_trio_al$caller_minus_capillary, levels = c("max_e_allele_diff", "min_e_allele_diff", "max_g_allele_diff", "min_g_allele_diff", "max_h_allele_diff", "min_h_allele_diff"))


fasta_with_capillary_trio_al%>%filter(key.x %in% unlist(split_keys[12])) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
geom_boxplot()+ theme_minimal()+ theme(panel.grid.minor = element_blank())+ xlab("locus")  +labs(color='Discrepancy') +ylab("Difference (num.repeats) from capillary")+ scale_y_continuous(breaks=seq(-15,0,1))


reference_graph_al = fasta_with_capillary_trio_al  %>% select(key.x, max_e_allele,min_e_allele,max_g_allele,min_g_allele,max_h_allele,min_h_allele, max_cap_allele, min_cap_allele,trf_num_repeats) %>% mutate(`EH-ref_1` = max_e_allele-trf_num_repeats,`EH-ref_2` = min_e_allele-trf_num_repeats,`GangSTR-ref_1` = max_g_allele-trf_num_repeats,`GangSTR-ref_2` = min_g_allele-trf_num_repeats, `HipSTR-ref_1` = max_h_allele-trf_num_repeats,`HipSTR-ref_2` = min_h_allele-trf_num_repeats, `capillary-ref_1` = max_cap_allele-trf_num_repeats, `capillary-ref_2` = min_cap_allele-trf_num_repeats) %>% gather(key = "caller_minus_capillary", value = "difference",11:18) 

reference_graph_al$caller_minus_capillary = factor(reference_graph_al$caller_minus_capillary, levels = c("EH-ref_1","EH-ref_2", "GangSTR-ref_1","GangSTR-ref_2", "HipSTR-ref_1", "HipSTR-ref_2", "capillary-ref_1", "capillary-ref_2"))

reference_graph_al%>%filter(key.x %in% unlist(split_keys[12])) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
geom_boxplot()+ theme_minimal()+ theme(panel.grid.minor = element_blank())+ xlab("locus")  +labs(color='Discrepancy') +ylab("Difference (num.repeats) from reference")+ scale_y_continuous(breaks=seq(-15,120,1))

```
# Offset calculations as per Gymrek 2014
```{r}
#calculating offsets as per Gymrek 2014 

offset_scoring = fasta_with_capillary_trio %>% select(cpg_id, key.x, e_allele_1, e_allele_2, g_allele_1, g_allele_2,h_allele_1, h_allele_2, capillary_allele_1, capillary_allele_2)

offset_scoring$e_allele_1 = as.numeric(offset_scoring$e_allele_1)
offset_scoring$e_allele_2 = as.numeric(offset_scoring$e_allele_2)
offset_scoring$g_allele_1 = as.numeric(offset_scoring$g_allele_1)
offset_scoring$g_allele_2 = as.numeric(offset_scoring$g_allele_2)

offset_scoring$big_e_allele = apply(offset_scoring[, 3:4 ], 1, max)
offset_scoring$small_e_allele = apply(offset_scoring[, 3:4 ], 1, min)

offset_scoring$big_g_allele = apply(offset_scoring[,5:6],1,max)
offset_scoring$small_g_allele = apply(offset_scoring[,5:6],1,min)

offset_scoring$big_h_allele = floor(apply(offset_scoring[,7:8],1,max))
offset_scoring$small_h_allele = floor(apply(offset_scoring[,7:8],1,min))

offset_scoring$big_capillary_allele = floor(apply(offset_scoring[,9:10],1,max))
offset_scoring$small_capillary_allele = floor(apply(offset_scoring[,9:10],1,min))
offset_scoring= offset_scoring %>% select(-e_allele_1, -e_allele_2, -g_allele_1, -g_allele_2, -h_allele_1, -h_allele_2, -capillary_allele_1, -capillary_allele_2)

#EH offsets

offset_scoring_eh = offset_scoring %>% mutate(big_e_allele_offset = big_e_allele - big_capillary_allele, small_e_allele_offset = small_e_allele-small_capillary_allele)
offset_scoring_eh = offset_scoring_eh %>% mutate(big_e_offset_score_label = case_when(big_e_allele_offset == small_e_allele_offset ~ 1, big_e_allele == small_e_allele  ~0.5, TRUE ~ 0.25))
offset_scoring_eh = offset_scoring_eh %>% mutate(small_e_offset_score_label = case_when(big_e_allele_offset == small_e_allele_offset ~ 1, big_e_allele == small_e_allele  ~0.5, TRUE ~ 0.25)) 
offset_scoring_eh=offset_scoring_eh %>% gather(key="offset_score_label", value="score",13:14)
offset_scoring_eh_big_allele = offset_scoring_eh %>% group_by(key.x, big_e_allele_offset) %>% filter(offset_score_label == "big_e_offset_score_label") %>%summarise(big_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(big_sum_score)) %>% filter(big_sum_score == max_score) %>% select(-max_score)

offset_scoring_eh_small_allele = offset_scoring_eh %>% group_by(key.x, small_e_allele_offset) %>% filter(offset_score_label == "small_e_offset_score_label") %>%summarise(small_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(small_sum_score)) %>% filter(small_sum_score == max_score) %>% select(-max_score)

offset_scoring_eh = merge(offset_scoring_eh_big_allele, offset_scoring_eh_small_allele)  %>% select(-big_sum_score, -small_sum_score)


#GangSTR offsets
offset_scoring_gangstr = offset_scoring %>% mutate(big_g_allele_offset = big_g_allele - big_capillary_allele, small_g_allele_offset = small_g_allele-small_capillary_allele)
offset_scoring_gangstr = offset_scoring_gangstr %>% mutate(big_g_offset_score_label = case_when(big_g_allele_offset == small_g_allele_offset ~ 1, big_g_allele == small_g_allele  ~0.5, TRUE ~ 0.25))
offset_scoring_gangstr = offset_scoring_gangstr %>% mutate(small_g_offset_score_label = case_when(big_g_allele_offset == small_g_allele_offset ~ 1, big_g_allele == small_g_allele  ~0.5, TRUE ~ 0.25)) 
offset_scoring_gangstr=offset_scoring_gangstr %>% gather(key="offset_score_label", value="score",13:14)

offset_scoring_g_big_allele = offset_scoring_gangstr %>% group_by(key.x, big_g_allele_offset) %>% filter(offset_score_label == "big_g_offset_score_label") %>%summarise(big_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(big_sum_score)) %>% filter(big_sum_score == max_score) %>% select(-max_score)

offset_scoring_g_small_allele = offset_scoring_gangstr %>% group_by(key.x, small_g_allele_offset) %>% filter(offset_score_label == "small_g_offset_score_label") %>%summarise(small_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(small_sum_score)) %>% filter(small_sum_score == max_score) %>% select(-max_score)

offset_scoring_g = merge(offset_scoring_g_big_allele, offset_scoring_g_small_allele)  %>% select(-big_sum_score, -small_sum_score)

#HipSTR offsets 

offset_scoring_hipstr = offset_scoring %>% mutate(big_h_allele_offset = big_h_allele - big_capillary_allele, small_h_allele_offset = small_h_allele-small_capillary_allele)
offset_scoring_hipstr = offset_scoring_hipstr %>% mutate(big_h_offset_score_label = case_when(big_h_allele_offset == small_h_allele_offset ~ 1, big_h_allele == small_h_allele  ~0.5, TRUE ~ 0.25))
offset_scoring_hipstr = offset_scoring_hipstr %>% mutate(small_h_offset_score_label = case_when(big_h_allele_offset == small_h_allele_offset ~ 1, big_h_allele == small_h_allele  ~0.5, TRUE ~ 0.25)) 
offset_scoring_hipstr=offset_scoring_hipstr %>% gather(key="offset_score_label", value="score",13:14)

offset_scoring_h_big_allele = offset_scoring_hipstr %>% group_by(key.x, big_h_allele_offset) %>% filter(offset_score_label == "big_h_offset_score_label") %>%summarise(big_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(big_sum_score)) %>% filter(big_sum_score == max_score) %>% select(-max_score)

offset_scoring_h_small_allele = offset_scoring_hipstr %>% group_by(key.x, small_h_allele_offset) %>% filter(offset_score_label == "small_h_offset_score_label") %>%summarise(small_sum_score = sum(score)) %>% ungroup() %>% group_by(key.x) %>% mutate(max_score = max(small_sum_score)) %>% filter(small_sum_score == max_score) %>% select(-max_score)

offset_scoring_h = merge(offset_scoring_h_big_allele, offset_scoring_h_small_allele)  %>% select(-big_sum_score, -small_sum_score)

# Merge offset across all 3 callers
offset_scoring_eh_g = merge(offset_scoring_eh, offset_scoring_g)
offset_scoring_trio = merge(offset_scoring_eh_g, offset_scoring_h)

#filter for offsets where there is agreement between 3 callers and also between alleles 
offset_scoring_trio_final = offset_scoring_trio %>% filter(big_e_allele_offset == big_h_allele_offset & big_h_allele_offset == big_g_allele_offset & big_g_allele_offset == small_g_allele_offset & small_g_allele_offset == small_e_allele_offset & small_e_allele_offset == small_h_allele_offset) %>% select(key.x, big_e_allele_offset) %>% rename(allele_offset = big_e_allele_offset)

#if hipstr has NA offset, we compare GangSTR and EH only 
offset_scoring_trio_final = rbind(offset_scoring_trio_final,offset_scoring_trio %>% filter(is.na(big_h_allele_offset) & big_e_allele_offset==big_g_allele_offset& small_g_allele_offset == small_e_allele_offset )%>% select(key.x, big_e_allele_offset) %>% rename(allele_offset = big_e_allele_offset))

```

```{r}
#apply offset to dataset 

fasta_with_capillary_trio_with_offset = merge(fasta_with_capillary_trio, offset_scoring_trio_final, all.x = TRUE)
fasta_with_capillary_trio_with_offset = fasta_with_capillary_trio_with_offset %>% mutate(allele_offset = case_when(is.na(allele_offset)~0, TRUE ~ allele_offset)) #if no offset was provided, assume it is 0

fasta_with_capillary_trio_with_offset = fasta_with_capillary_trio_with_offset %>% mutate(
                                      capillary_allele_1 = capillary_allele_1+ allele_offset,
                                      capillary_allele_2 = capillary_allele_2 + allele_offset, 
                                      capillary_alleles = floor(capillary_allele_1) +floor(capillary_allele_2))
```

#Concordance results with offset applied 
```{r}

#concordance with capillary calls after offset is applied 
(fasta_with_capillary_trio_with_offset %>% filter(e_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio_with_offset%>% count())
(fasta_with_capillary_trio_with_offset %>% filter(h_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio_with_offset%>% count())
(fasta_with_capillary_trio_with_offset %>% filter(g_alleles== capillary_alleles) %>%count())/ (fasta_with_capillary_trio_with_offset %>% count())

```

# Misc: alternative approach to calculating offset
```{r}
...
# ROC visualisation 
```{r}
num_e_concord = fasta_with_capillary_trio_with_offset %>% group_by(key.x)%>% filter(e_alleles== capillary_alleles) %>% summarise(num_e_concord=n())
num_by_key = fasta_with_capillary_trio_with_offset%>% group_by(key.x)%>% summarise(num_by_key=n())
e_roc_data = merge(num_by_key, num_e_concord, all.x = TRUE) %>% mutate(num_e_concord = case_when(is.na(num_e_concord)~ 0, TRUE ~ num_e_concord)) %>% mutate(e_concord_rate = num_e_concord/num_by_key) %>% select(e_concord_rate) %>% arrange(e_concord_rate)

num_h_concord = fasta_with_capillary_trio_with_offset %>% group_by(key.x)%>% filter(!is.na(h_alleles)) %>% filter(h_alleles== capillary_alleles) %>% summarise(num_h_concord=n()) 
h_roc_data = merge(num_by_key, num_h_concord, all.x = TRUE) %>% mutate(num_h_concord = case_when(is.na(num_h_concord)~ 0, TRUE ~ num_h_concord))%>% mutate(h_concord_rate = num_h_concord/num_by_key) %>% select(h_concord_rate) %>%arrange(h_concord_rate) 


num_g_concord = fasta_with_capillary_trio_with_offset %>% group_by(key.x)%>% filter(!is.na(g_alleles)) %>% filter(g_alleles== capillary_alleles) %>% summarise(num_g_concord=n()) 
g_roc_data = merge(num_by_key, num_g_concord, all.x = TRUE) %>% mutate(num_g_concord = case_when(is.na(num_g_concord)~ 0, TRUE ~ num_g_concord))%>% mutate(g_concord_rate = num_g_concord/num_by_key) %>% select(g_concord_rate) %>% arrange(g_concord_rate) 

index = c(1:351)

roc_data = cbind(index,e_roc_data, h_roc_data, g_roc_data) %>% rename("ExpansionHunter" = e_concord_rate, "HipSTR" = h_concord_rate, "GangSTR" = g_concord_rate) %>% gather(key= "Caller",value = "concord_rate",2:4) %>% mutate(index = index/351*100, concord_rate = concord_rate*100) 

roc_data$Caller = factor(roc_data$Caller , levels =c('ExpansionHunter', 'HipSTR','GangSTR'))


roc_data %>% ggplot(aes(x = index, y=concord_rate, color = Caller)) + geom_line() + theme_minimal()+xlab("Sensitivity (%)") +ylab("Accuracy (%)")
```
...
motif_graph = fasta_with_capillary_trio %>% select(trf_period, e_alleles,g_alleles,h_alleles,capillary_alleles, key.x) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles, `HipSTR-capillary` = h_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",7:9) 
...
keys_minus_2 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-2) %>% distinct(key.x)


keys_minus_4 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-4) %>% distinct(key.x)

keys_minus_6 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-6) %>% distinct(key.x)

keys_minus_8 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-8) %>% distinct(key.x)
keys_minus_10 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-10) %>% distinct(key.x)
keys_minus_12 = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==-12) %>% distinct(key.x)
keys_plus_2= fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==2) %>% distinct(key.x)
keys_plus_4= fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==4) %>% distinct(key.x)
...
keys_100_concord = fasta_with_capillary_trio %>% select(key.x, e_alleles,g_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(max_EH_capillary = max(`EH-capillary`), min_EH_capillary = min(`EH-capillary`)) %>% filter(min_EH_capillary==max_EH_capillary & min_EH_capillary ==0) %>% distinct(key.x)
