...
unlist(split_keys[1]) 
fasta_with_capillary_eh_gangstr %>% select(key.x, e_alleles,g_alleles,h_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles, `HipSTR-capillary` = h_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",6:8)  %>%filter(key.x %in%unlist(split_keys[9])  ) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
...
fasta_with_capillary_eh_gangstr %>% filter(key.x == "GAT004") %>% select(PCR_summed_diffs)
```
