...
read.csv("~/Downloads/str_410_sgdp_loci_data-frames_410_sgdp_loci_hipstr.csv") %>% filter(h_id == "MFD424-TTTA003") %>% select(h_ref)%>% mutate(nchar = nchar(h_ref))

%>% select(hgdp_sample_id, key, h_allele_1, h_allele_2, h_alleles, h_gbs)
...
fasta_with_capillary_eh_gangstr = merge(fasta_with_capillary_eh_gangstr, hipstr, by =c("cpg_id", "key"), all.x = TRUE)
fasta_with_capillary_eh_gangstr %>% filter(key == "MFD424-TTTA003")
```
...
(fasta_with_capillary_eh_gangstr %>% filter(h_gbs== PCR_summed_diffs) %>%count())/ (fasta_with_capillary_eh_gangstr %>% count())
