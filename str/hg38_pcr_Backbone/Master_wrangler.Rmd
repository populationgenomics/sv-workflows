...
trf_output = fromJSON(file = "2. TRF_find_motif/filtered_trf_output_debugged _modified.txt") %>% as.data.frame() %>% gather() 
...
write.table(trf_input, file = "trf_input_antisense_debugged.txt", sep = "\n", row.names = FALSE, quote = FALSE)

fasta = fasta %>% mutate(catalog_start_coordinate_1_based = catalog_start_coordinate_0_based+1)
```

```{r}
trf_output_antisense = read.csv("3. redefining_motifs_on_antisense/trf_output_antisense_debugged.txt") %>% separate(sequence, c("chr","coordinates" ), sep = ":") %>% separate(coordinates, c("catalog_start_coordinate_0_based", "catalog_end_coordinate"), sep = "-") %>% rename("updated_motif" = "motif")
```
...
```{r}
hipstr = read.csv("~/Downloads/str_410_sgdp_loci_data-frames_410_sgdp_loci_hipstr (1).csv")%>% separate(h_gb, c("h_gb_1", "h_gb_2"), sep = "\\|") %>% mutate(h_gb_1 = as.numeric(h_gb_1), h_gb_2 = as.numeric(h_gb_2))  %>% rename("key"= h_id,"hgdp_sample_id"= sample_id) %>% rename("end" = h_end, "chr" = h_chr)
...
fasta_with_capillary_eh_gangstr %>% filter(key.x == "GATA7G07")%>% select(h_allele_1, h_allele_2, capillary_allele_1, capillary_allele_2, trf_num_repeats)
fasta_with_capillary_eh_gangstr %>% distinct(nameInGenotypeDataFile,)
...
fasta_with_capillary_eh_gangstr  %>% select(key.x, e_alleles,g_alleles,h_alleles,capillary_alleles) %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles, `HipSTR-capillary` = h_alleles-capillary_alleles) %>% gather(key = "caller_minus_capillary", value = "difference",6:8)  %>%filter(key.x %in% unlist(split_keys[5])) %>%ggplot(aes(x = key.x, y= difference, colour =caller_minus_capillary, alpha = 0.6,position = 'dodge' ))  +
...
fasta_with_capillary_eh_gangstr%>% filter(key.x == "AFMb035xb9") %>% select(e_alleles, capillary_alleles) %>% filter(e_alleles == capillary_alleles)
```
...
fasta_with_capillary_eh_gangstr_edited = fasta_with_capillary_eh_gangstr %>% mutate(`EH-capillary` = e_alleles-capillary_alleles, `GangSTR-capillary` = g_alleles-capillary_alleles,`HipSTR-capillary` = h_alleles-capillary_alleles) %>% group_by(key.x) %>% mutate(mode_EH_capillary = getmode(`EH-capillary`),mode_GangSTR_capillary = getmode(`GangSTR-capillary`), mode_HipSTR_capillary = getmode(`HipSTR-capillary`)) %>% ungroup() %>% mutate(is_mode_EH_capillary = case_when(mode_EH_capillary == `EH-capillary` ~ 1, TRUE ~0), is_mode_GangSTR_capillary = case_when(mode_GangSTR_capillary == `GangSTR-capillary` ~ 1, TRUE ~0), is_mode_HipSTR_capillary = case_when(mode_HipSTR_capillary == `HipSTR-capillary` ~ 1, TRUE ~0)) %>% group_by(key.x) %>% mutate(mode_EH_capillary_proportion = sum(is_mode_EH_capillary)/n(), mode_GangSTR_capillary_proportion = sum(is_mode_GangSTR_capillary)/n(),mode_HipSTR_capillary_proportion = sum(is_mode_HipSTR_capillary)/n()) %>% mutate(capillary_alleles = case_when(mode_GangSTR_capillary==mode_EH_capillary & mode_EH_capillary == mode_HipSTR_capillary & mode_EH_capillary_proportion >0.85 &mode_EH_capillary_proportion >0.85 &mode_GangSTR_capillary_proportion >0.75 &mode_GangSTR_capillary!=0 ~ NA, TRUE ~ capillary_alleles))
