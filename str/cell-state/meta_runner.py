...
analysis-runner --dataset "tenk10k" --description "meta results runner" --access-level "test" \
    --output-dir "str/cellstate/stratified" \
    meta_runner.py --results-dir-1=gs://cpg-tenk10k-test-analysis/str/cellstate/stratified/bioheart/results \
    --results-dir-2=gs://cpg-tenk10k-test-analysis/str/cellstate/stratified/tob/results \
...
def run_meta_gen(input_dir_1, input_dir_2, cell_type, chr, gene, pathway, activity_level):
    """
    ...
    d1 = pd.read_csv(f'{input_dir_1}/{pathway}/{activity_level}/{cell_type}/{chr}/{gene}_100000bp.tsv', sep='\t')
    d2 = pd.read_csv(f'{input_dir_2}/{pathway}/{activity_level}/{cell_type}/{chr}/{gene}_100000bp.tsv', sep='\t')
    ...
    # write to GCS
    pd_meta_df.to_csv(
        f'{output_path(f"meta_results/{pathway}/{activity_level}/{cell_type}/{chr}/{gene}_100000bp_meta_results.tsv", "analysis")}',
        sep='\t',
        index=False,
    )
    ...
    for cell_type in cell_types.split(','):
        for chromosome in chromosomes.split(','):
            for activity_level in ['high', 'medium', 'low']:
                ...
                gene_file_path_1 = to_path(f'{gene_list_dir_1}/{pathway}/{activity_level}/{cell_type}/{chromosome}_{cell_type}_gene_list.json')
                gene_file_path_2 = to_path(f'{gene_list_dir_2}/{pathway}/{activity_level}/{cell_type}/{chromosome}_{cell_type}_gene_list.json')
                ...
                for gene in intersected_genes:
                    if to_path(
                        ...
                        continue
                    j = get_batch(name='compute_meta').new_python_job(name=f'compute_meta_{cell_type}_{chromosome}_{gene}_{activity_level}')
                    j.cpu(0.25)
                    if always_run:
                        j.always_run()
                    j.image(image_path('r-meta'))
                    j.call(run_meta_gen, results_dir_1, results_dir_2, cell_type, chromosome, gene,pathway, activity_level)
