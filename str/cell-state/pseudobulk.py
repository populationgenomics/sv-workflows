...
analysis-runner --config pseudobulk.toml --access-level test --dataset tenk10k --image australia-southeast1-docker.pkg.dev/cpg-common/images/scanpy:1.9.3 --description "pseudobulk" --output-dir "str/cellstate/input_files/Feb2026/meanpool/stratified/tob/pseudobulk" python3 pseudobulk.py
...
def pseudobulk(input_file_path, id_file_path, target_sum, min_pct,cell_type,pathway):
    ...
    for activity in ['low', 'medium', 'high']:
        ...
        data_df.to_csv(
            output_path(f'{cell_type}/{cell_type}_chr{chrom_num}_{activity}_{pathway}_pseudobulk.csv'),
            index=False,
        )
