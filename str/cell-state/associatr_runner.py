    ...
    for celltype in get_config()['associatr']['celltypes'].split(','):
        for chromosome in get_config()['associatr']['chromosomes'].split(','):
            ...
            for activity_level in ['high', 'medium', 'low']:
                ...
                pathway = get_config()['associatr']['pathway']
                with to_path(f'{gene_list_dir}/{pathway}/{activity_level}/{celltype}/{chromosome}_{celltype}_gene_list.json').open('r') as file:
                ...
                for gene in pseudobulk_gene_names:
                    ...
                    if to_path(
                        output_path(
                            f'results/{pathway}/{activity_level}/{celltype}/{chromosome}/{gene}_{cis_window_size}bp.tsv',
                            'analysis',
                        ),
                    ).exists():
                        continue

                    gene_cis_window_file = f'{cis_window_dir}/{pathway}/{activity_level}/{celltype}/{chromosome}/{gene}_{cis_window_size}bp.bed'
                    ...
                    gene_pheno_cov = b.read_input(f'{pheno_cov_numpy_dir}/{pathway}/{activity_level}/{celltype}/{chromosome}/{gene}_pheno_cov.npy')

                    # run associaTR job on the gene
                    associatr_job = b.new_job(name=f'Run associatr on {gene} [{celltype};{chromosome}] {activity_level}')
                    ...
                    b.write_output(
                        ...
                        output_path(
                            f'results/{pathway}/{activity_level}/{celltype}/{chromosome}/{gene}_{cis_window_size}bp',
                            'analysis',
                        ),
                    )
