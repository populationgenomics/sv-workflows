...
def compute_storey(input_dir, cell_type, chromosomes, gene_level_correction):
    ...
    for chromosome in chromosomes.split(','):
        ...
        gene_pval_files = list(
            to_path(f'{input_dir}/{gene_level_correction}/{cell_type}/chr{chromosome}').glob('*.tsv'),
        )
        if first_iteration:
...
def main(input_dir, cell_types, chromosomes, gene_level_correction):
    ...
    for cell_type in cell_types.split(','):
        j = get_batch(f'compute_storey {gene_level_correction}').new_python_job(name=f'compute_storey_{cell_type}')
        j.cpu(1)
