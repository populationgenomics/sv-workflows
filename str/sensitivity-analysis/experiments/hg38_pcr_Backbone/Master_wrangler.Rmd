...
sgdp_metadata = read.table("SGDP_metadata.279public.21signedLetter.44Fan.samples.txt", header = TRUE, sep = "\t") %>% mutate(SGDP_ID=gsub("-", ".", SGDP_ID)) %>% select(SGDP_ID, Sample_ID)

sgdp_metadata %>% filter(grepl("S_Abkhasian",SGDP_ID))
...
```{r}
eh = read.csv("~/Downloads/str_sensitivity-analysis_sgdp_410_loci_trimmed_eh_trimmed_0_based.csv") %>% select(sample_id, chr, start, end, e_allele_1, e_allele_2) %>% rename("catalog_start_coordinate_0_based" = start)

#%>% mutate(catalog_start_coordinate_0_based = as.numeric(catalog_start_coordinate_1_based)-1)

gangstr = read.delim("~/Downloads/str_sensitivity-analysis_sgdp_410_loci_trimmed_gangstr_trimmed_1_based.tsv", sep = "\t") %>% select(sample_id, chr, start, g_allele_1, g_allele_2)%>% rename("catalog_start_coordinate_1_based" = start) %>% mutate(catalog_start_coordinate_0_based = as.numeric(catalog_start_coordinate_1_based)-1)
