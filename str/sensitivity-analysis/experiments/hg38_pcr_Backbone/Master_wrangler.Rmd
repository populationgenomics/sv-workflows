...
#write.table(rbind(fermikit_bed_1, fermikit_bed_2), file = "fermikit_hg38.bed", sep = "\t", row.names = FALSE, quote = FALSE)
```
...
#fermikit_liftover = read.table("fermikit_hg38_liftover.bed", header = F, sep = "\t")
#fermikit_input_hg19 = fermikit_liftover %>% mutate(V1= gsub("chr","", V1)) %>% select(V1, V2, V3)
#write.table(fermikit_input_hg19, file = "fermikit_hg19.bed", sep = "\t", row.names = FALSE, quote = FALSE)
```
...
```{r}
fermikit_indels = read.table("263.intersect_PCR.vcf", sep = "\t", header = TRUE) %>% gather("sample_id", "gt", 10:15) %>% rename("start_coordinate_indel_hg19" = POS, "chr" = CHROM) %>% mutate(end_coordinate_indel_hg19 = start_coordinate_indel_hg19 +nchar(REF)-1,chr = paste("chr", chr, sep = "") ) %>% select(chr, start_coordinate_indel_hg19, end_coordinate_indel_hg19, sample_id, gt)

#fermikit_indels %>% select(chr, start_coordinate, end_coordinate) %>% write.table(file = "fermikit_indel_coordinates_hg19.bed", sep = "\t", row.names = FALSE, quote = FALSE) #note hg19 - must liftover back to hg38 to interseect 

fermikit_indels_hg38 = read.table("fermikit_indel_coordinates_hg38_liftover.bed", sep = "\t", header = FALSE) %>% select(-V5)%>% rename("chr" = V1, "start_coordinate_indel_hg38" = V2, "end_coordinate_indel_hg38"= V3) %>% separate(V4, c("chr_hg19", "coordinates"), sep = ":") %>% separate(coordinates, c("start_coordinate_indel_hg19", "end_coordinate_indel_hg19"), sep = "-") %>% mutate(start_coordinate_indel_hg19 = as.numeric(start_coordinate_indel_hg19)-1) %>% distinct()
...
#fermikit_indels_hg38 %>% select(chr, start_coordinate_indel_hg38, end_coordinate_indel_hg38) %>% write.table(file = "fermikit_indel_coordinates_hg38.bed", sep = "\t", row.names = FALSE, quote = FALSE) 
...
indel_fasta_intersect = read.table("indel_fasta_intersect.bed", sep ="\t", header = FALSE) %>% select(-V4)%>% rename("chr" = V1,"start_coordinate_indel_hg38" = V2, "end_coordinate_indel_hg38" = V3, "start_coordinate_PCR" = V5, "end_coordinate_PCR" = V6) %>% distinct(chr, start_coordinate_indel_hg38, end_coordinate_indel_hg38, start_coordinate_PCR, end_coordinate_PCR)

fermikit_indels_hg38_clean = merge(fermikit_indels_hg38, indel_fasta_intersect, by = c("chr", "start_coordinate_indel_hg38", "end_coordinate_indel_hg38"), all.x = TRUE) %>% rename("indel_gt"= gt)%>% mutate(indel_gt = case_when (indel_gt == "0/0" ~0, TRUE ~ 1)) %>% group_by(sample_id, start_coordinate_PCR, end_coordinate_PCR) %>% summarise(indel_gt = sum(indel_gt)) %>% filter(!is.na(start_coordinate_PCR) & !is.na(end_coordinate_PCR))
```

```{r}
capillary_genotypes = read.csv("capillary_gt/capillary_genotypes_with_locusNames_rosenberg2005.csv") %>% select(-X,-locus_id, -population_id, - geographic_population, -predefined_region)
...
## working with fermikit calls - ie only 6 samples
fasta_with_capillary = fasta_with_capillary %>% filter(sample_id %in% c("HGDP00530","HGDP00526","HGDP00783", "HGDP00785","HGDP00928","HGDP00932"))
excluded_calls = merge(fasta_with_capillary, fermikit_indels_hg38_clean, by = c("sample_id", "start_coordinate_PCR", "end_coordinate_PCR"), all.x = TRUE)%>% filter(!is.na(indel_gt)| indel_gt!= 0) 
```

```{r}
eh = read.csv("str_sensitivity-analysis_data_frames_trimmed_coordinates_hg38_backbone_0_based_hg38_backbone_trimmed_0_based_eh.csv") %>% select(sample_id, chr, start, end, e_allele_1, e_alelle_2) %>% rename("catalog_start_coordinate_0_based" = start)

#%>% mutate(catalog_start_coordinate_0_based = as.numeric(catalog_start_coordinate_1_based)-1)

gangstr = read.delim("str_sensitivity-analysis_data_frames_trimmed_coordinates_hg38_backbone_0_based_hg38_backbone_trimmed_0_based_gangstr.tsv", sep = "\t") %>% select(sample_id, chr, start, g_allele_1, g_allele_2)%>% rename("catalog_start_coordinate_0_based" = start) 
#%>% mutate(catalog_start_coordinate_0_based = as.numeric(catalog_start_coordinate_1_based)-1)
...
fasta_with_capillary_eh_gangstr = anti_join(fasta_with_capillary_eh_gangstr, excluded_calls,by = c("sample_id", "start_coordinate_PCR", "end_coordinate_PCR"))
