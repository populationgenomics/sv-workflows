...
For example:
analysis-runner --access-level test --dataset tob-wgs --description 'merge_str_prep' --output-dir 'str/5M_run_combined_vcfs/merge_str_prep' merge_str_prep.py --caller=eh --input-dir=gs://cpg-tob-wgs-test-analysis/str/5M_run_combined_vcfs CPGtester
        ...
        if caller == 'eh':
            bcftools_job.declare_resource_group(
                vcf_sorted={
                    ...
                    'sorted.vcf.gz': '{root}.sorted.vcf.gz',
                }
            )
            bcftools_job.command(
                ...
                tabix -p vcf {bcftools_job.vcf_sorted['reheader.vcf.gz']}

                bcftools sort {bcftools_job.vcf_sorted['reheader.vcf.gz']}  > {bcftools_job.vcf_sorted['sorted.vcf.gz']}

                tabix -p vcf {bcftools_job.vcf_sorted['sorted.vcf.gz']}
