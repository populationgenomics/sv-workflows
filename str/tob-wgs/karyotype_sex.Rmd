...
```{r}
data = data %>% select(s,sex_karyotype,external_id)
data %>% distinct(sex_karyotype)
```

```{r}
data = data%>%select(s, sex_karyotype, external_id)
data = data %>% filter(!(s %in% c("NA12878", "NA12891","NA12892","syndip")))
write.csv(data, "karyotype_sex_mapping.csv")
```
