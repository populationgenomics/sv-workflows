...
For example:
analysis-runner --access-level standard --dataset hgdp --description 'hipstr run' --output-dir 'str/sensitivity-analysis/hipstr/untrimmed_coordinates_1_based' hipstr_runner_hgdp.py --variant-catalog=gs://cpg-hgdp-test/str/untrimmed_coordinates_resources/hg38_hipstr_catalog_untrimmed_coordinates.bed --input-dir=gs://cpg-hgdp-test/cram/nagim --external-wgs-ids HGDPXXXX HGDPXXXXY --output-file-name hipster_90_genomes
...
def main(
    ...
    # Initializing Batch 
    #b = get_batch()
    b = hb.Batch(backend=None, default_image=os.getenv('DRIVER_IMAGE'))
